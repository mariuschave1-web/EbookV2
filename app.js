...
function parsePrescription(prescription, defaultRest) {
  ...
  try {
    var parts = prescription.split(/[×x]/);
    var sets = parseInt(parts[0].trim()) || 3;
    var repStr = (parts[1] || "8").trim().replace(/\s+/g, "");
    return { sets: Math.min(sets, 6), reps: repStr, rest: defaultRest || 90 };
  } catch (_) {
    return { sets: 3, reps: "8", rest: defaultRest || 60 };
  }
}
...
var ENGINE_NAME_TO_FR = {
  ...
  "Wall HSPU":"Pompes en appui renversé mur","Deficit Wall HSPU":"Pompes renversées mur déficit",
  ...
  "Band Assisted Maltese Hold":"Maltese assisté (bande)","Maltese Hold":"Maltese hold",
  ...
  "One Arm Planche (Imperfect Hold)":"OAP (forme imparfaite)","One Arm Planche Hold":"OAP hold",
...
function isWeightedExercise(ex) {
  ...
  // Check if name contains "lesté" or "weighted"
  if (name.indexOf("lesté") >= 0 || name.indexOf("lestée") >= 0 || name.indexOf("lestés") >= 0 || name.indexOf("lestées") >= 0) return true;
  ...
  if (ex.tags && Array.isArray(ex.tags)) {
    for (var j = 0; j < ex.tags.length; j++) {
      if (WEIGHTED_TAGS.indexOf(ex.tags[j]) >= 0) return true;
    }
  }
...
function getBodyweightAlternative(ex) {
  ...
  var name = (ex.name || "").replace(/\s*lestée?s?\s*/gi, " ").replace(/\s*weighted\s*/gi, " ").trim();
...
function isProgramCompatibleWithTrainingMode(prog, trainingMode) {
  ...
  if (trainingMode === "pdc") {
    ...
    if (name.indexOf("lesté") >= 0 || name.indexOf("lestée") >= 0 || name.indexOf("weighted") >= 0) return false;
    if (name.indexOf("force lestée") >= 0) return false;
  }
  return true;
}
...
function normalizeEngineExercise(engineEx, defaultRest) {
  ...
  try {
    ...
    return {
      ...
      sets: parsed.sets, reps: parsed.reps, rest: parsed.rest,
      cues: Array.isArray(engineEx.coachingCues) ? engineEx.coachingCues.slice(0,2) : [],
      errors: Array.isArray(engineEx.commonErrors) ? engineEx.commonErrors.slice(0,1) : [],
...
function normalizeEngineWorkout(engineWorkout, sessionName) {
  ...
  try {
    ...
    return {
      ...
      _notes: engineWorkout.notes || [], _cooldown: engineWorkout.cooldown || []
    };
  } catch (_) { return null; }
}
...
function generateSmartWorkoutFromProfile(appState, readiness) {
  ...
  try {
    ...
    var zoneFR = zone==="high"?"Intensif":zone==="low"?"Récup":"Standard";
...
function generateSmartWeekFromProfile(appState, days) {
  ...
  try {
    ...
    var sessions = [];
    var focusNames = {
      "priority":"Figure prioritaire","secondary_1":"Secondaire","secondary_2":"Renforcement",
      "basics_and_weak":"Fondamentaux","full_body":"Full body","active_rest":"Récup active"
    };
...
var EXERCISE_MEDIA = {
  ...
  "Pseudo planche push-up": svgAnim("pseudoplanche", '<svg viewBox="0 0 200 140" xmlns="http://www.w3.org/2000/svg"><style>.pp{animation:ppMove 1.8s ease-in-out infinite;transform-origin:100px 85px}@keyframes ppMove{0%,100%{transform:translateY(0)}45%,55%{transform:translateY(18px)}}</style><rect x="0" y="0" width="200" height="140" fill="#0E0D18" rx="16"/><line x1="20" y1="110" x2="180" y2="110" stroke="#252345" stroke-width="2"/><g class="pp"><rect x="52" y="72" width="96" height="14" rx="7" fill="#FF6B35" opacity="0.85"/><circle cx="155" cy="65" r="11" fill="#F0EEF8" opacity="0.9"/></g><text x="100" y="130" text-anchor="middle" font-size="9" fill="#4B4870" font-family="system-ui">PSEUDO PLANCHE PU</text></svg>'),
      ...
      guide.technique&&e("div",{id:"gsec-technique"},
        e(GSection,{title:"Technique",icon:"⚙️"},
          ...
          guide.technique.grip_options&&e("div",{style:{marginTop:12}},
            e("div",{style:{fontSize:11,fontWeight:700,color:T.accent,marginBottom:8}},"GRIPS / SUPPORTS"),
            guide.technique.grip_options.map(function(g,i){
              return e("div",{key:i,style:{background:T.card,borderRadius:14,padding:"12px 14px",marginBottom:8}},
                e("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center"}},
                  ...
                  g.difficulty&&e("span",{style:{fontSize:10,color:T.textTert}},"Diff: "+g.difficulty+"/4")),
          ...
          guide.common_mistakes.map(function(m,i){
            ...
            return e("div",{key:i,style:{background:T.card,borderRadius:14,padding:"14px",marginBottom:8}},
              ...
              m.fix&&e("div",{style:{background:T.accentGlow,borderRadius:10,padding:"8px 12px"}},
                e("span",{style:{fontSize:11,fontWeight:600,color:T.accent}},"✦ Fix : "),
...
const PROGRAMS = [
  ...
  { id:"p3", name:"Prépa Muscle-Up", tagline:"Les fondations du roi des mouvements",
    level:"Intermédiaire", weeks:8, sessionsPerWeek:4, avgMin:50, pro:false, color:T.gold, icon:"👑",
    sessions:[
      { name:"Séance Pull explosif", ex:[
        {name:"Tractions pronation",cat:"Pull",sets:5,reps:"5",rest:120},
        {name:"High pull-up explosif",cat:"Pull",sets:4,reps:"3-4",rest:120},
        {name:"Dips barre droite",cat:"Skills",sets:3,reps:"6-8",rest:90},
        {name:"Suspension false grip",cat:"Skills",sets:4,reps:"15s",rest:60}]},
    ]},
...
function IntensityBadge(props){
  ...
  var labels = { low:"Récup", medium:"Standard", high:"Intensif" };
  ...
  return e("div",{style:{display:"flex",alignItems:"center",gap:5}},
    ...
    e("span",{style:{fontSize:10,fontWeight:700,color:col,letterSpacing:1}},(labels[zone]||"IA").toUpperCase()));
}
// Modal wrapper
...
function SupportModal(props){
  ...
  var faqs = [
    ...
    ["Comment fonctionne la progression ?","KOVA utilise des arbres de progression par mouvement avec des critères de déblocage (ex : 10 tractions propres pour passer aux variantes avancées). La progression suit un cycle : force → volume → forme → palier supérieur."],
...
function formatPrescription(rawReps, exName) {
  ...
  // Detect concatenated engine strings and split them
  // e.g. "qualityattemptswithfull5minrestbetween" 
  r = r.replace(/qualityattempts?/gi, "tentatives propres")
       .replace(/withfull/gi, " · repos complet")
       .replace(/withrest/gi, " · repos")
       .replace(/minrestbetween/gi, " min entre séries")
       .replace(/minrest/gi, " min repos")
       .replace(/srest/gi, "s repos")
       .replace(/secrest/gi, "s repos")
       .replace(/maxhold/gi, "max hold")
       .replace(/maxreps/gi, "max reps")
       .replace(/deadstop/gi, "dead stop");
  
  // Determine type from content
  var lc = r.toLowerCase();
  var isHold = /^\d+\s*s$/i.test(r) || lc.indexOf("hold") >= 0 || lc.indexOf("maintien") >= 0 || lc.indexOf("sec") >= 0;
  ...
  // Also detect holds from exercise names
  ...
  // Format hold exercises
  if ((isHold || (isHoldExercise && /^\d+/.test(r))) && !isAttempt) {
    var seconds = r.replace(/[^\d]/g, "");
    if (seconds) return { value: seconds + "s", label: "MAINTIEN", detail: null };
    if (isMax) return { value: "MAX", label: "MAINTIEN", detail: null };
  }
  
  // Format attempts
  if (isAttempt) {
    // Extract any extra details
  ...
  // Format max
  if (isMax && !isHold) {
    return { value: "MAX", label: "REPS", detail: r.indexOf("repos") >= 0 ? r : null };
  }
  ...
  if (/^\d+-\d+s?$/.test(r)) {
    if (r.indexOf("s") >= 0 || isHoldExercise) {
      return { value: r, label: "MAINTIEN", detail: null };
    }
  ...
  // Simple number
  if (/^\d+$/.test(r)) {
    if (isHoldExercise) return { value: r + "s", label: "MAINTIEN", detail: null };
    return { value: r, label: "REPS", detail: null };
  }
  
  // Complex strings — clean and show
  if (r.length > 20) {
    // Too long for main display - split into value + detail
